<nav x-data="{ open: false }" class="bg-white border-b-4 border-pop-candy/50 sticky top-0 z-50 shadow-soft">
    ...
    <!-- TAMPILKAN FOTO PROFIL JIKA ADA (Hanya User) -->
    @if(Auth::user()->role === 'user' && Auth::user()->profile_photo)
        <img src="{{ asset('storage/' . Auth::user()->profile_photo) }}" class="w-8 h-8 rounded-full object-cover border-2 border-pop-lime mr-2">
    @endif
